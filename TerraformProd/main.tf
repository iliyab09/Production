...
resource "azurerm_postgresql_server" "ps_prod" {
  name                = "ps-db-prod"
  location            = var.ResourceGroup_location
  resource_group_name = var.ResourceGroup_name
...
resource "azurerm_postgresql_firewall_rule" "postgres_firewall" {
  name                = "prod_rule"
  resource_group_name = var.ResourceGroup_name
  server_name         = azurerm_postgresql_server.ps_prod.name
  start_ip_address    = data.azurerm_public_ip.ip_data.ip_address
  end_ip_address      = data.azurerm_public_ip.ip_data.ip_address
